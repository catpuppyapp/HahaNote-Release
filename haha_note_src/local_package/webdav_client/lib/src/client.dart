...
class Client {
  ...
  Future<List<File>> readDir(String path, [CancelToken? cancelToken]) async {
    ...
    return WebdavXml.toFiles(path, str, isReadDir: true);
  }

  /// Read a single files properties
  Future<File> readProps(String path, [CancelToken? cancelToken]) async {
    // 若是文件，不用加末尾/，所以这里不该处理
    // 参见：https://github.com/flymzero/webdav_client/issues/62
    // path = fixSlashes(path);
    ...
    String str = resp.data;
    return WebdavXml.toFiles(path, str, skipSelf: false, isReadDir: false).first;
  }
