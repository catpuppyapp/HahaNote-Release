...
class WebdavXml {
  ...
  static List<File> toFiles(String path, String xmlStr, {skipSelf = true, required bool isReadDir}) {
    var files = <File>[];
      ...
      for (var propstat in props) {
        ...
        if (findElements(propstat, 'status').single.text.contains('200')) {
          ...
          for (var prop in findElements(propstat, 'prop')) {
            ...
            // var filePath = path + name + (isDir ? '/' : ''); // 读取目录信息，后面文件名会重复

            var filePath = isReadDir ? '$path$name' : path; //感觉dir末尾也没必要加/，所以没加，不是忘了
